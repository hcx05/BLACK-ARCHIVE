...
9. **每個推理節點都要能回答「為什麼是這樣」跟「為什麼不是別的答案」，不能只靠語感**（第二十一輪起）：每個推理節點在這份文件裡都要寫清楚兩件事——(a) 支持結論的具體證據是什麼；(b) 最可能的替代解釋為什麼站不住腳，不能只寫「所以答案是 X」就結束。如果證據真的不足以排除替代解釋，這個節點就必須誠實標成「目前無法確定，需要更多證據」（像 Farrow 的結局、3.4 節的官方稽核矛盾那樣），不能為了讓遊戲看起來「都有答案」硬做出一個實際上站不住腳的結論。細節見 `../Answer/evidence-map.md` 第二十一輪，逐一節點的具體寫法見 4.5 節跟下方 LONGSHORE 身分鏈段落。
...
**為什麼「純屬巧合」站不住腳，但「官方在說謊」也還站不住腳**：三個獨立來源（新人口頭傳承、系統人員私人待辦、Records 正式公告）如果真的各自獨立寫成，措辭不會近乎逐字相同——這是巧合說法的第一個破口。但即使措辭統一證明了「有人在統一口徑」，也還沒證明「統一的口徑是假的」：也可能真的就是一次批次匯入雜訊，只是剛好被寫進標準話術範本、大家照樣複製貼上，這種官僚惰性完全合理，不需要陰謀就能解釋統一措辭本身。真正讓「純屬雜訊」站不住腳的，是第 3 點的統計訊號（剛好黏著 LONGSHORE 名單裡的名字、剛好持續好幾年）——雜訊不會系統性地只黏著特定幾個名字。這正是這個節點停在「懷疑」而不是「結論」的原因：現有證據排除得掉「這是隨機雜訊」，但排除不掉「這是官僚惰性複製貼上，剛好蓋住了真正有問題的案子」與「這是刻意的統一說法」兩者之間的差別——這個差別要到 Act II/III 才有實質證據可以判斷。
...
**為什麼這裡誠實地無法確定，而不是遊戲偷懶不寫答案**：這筆結案回應只有結論（「查過了，是假影」），沒有留下當年實際調查過程的任何紀錄——玩家看不到 2540 年 Compliance 到底比對了什麼、查了多深。「失職」（隨便看一眼就結案）跟「刻意淡化」（其實查出東西但選擇不寫）在玩家目前手上的證據裡，會產生**一模一樣的輸出**：一句簡短的官方結論，沒有過程紀錄可以拿來分辨這兩種可能。這是跟 4.5 節（07-B 身分）根本不同的情況——07-B 那裡玩家有殖民地/年齡兩個獨立資料點可以交叉比對，這裡玩家只有結論、沒有第二個獨立可比對的資料點，所以誠實的結論是「無法確定」，不是隨便選一個聽起來合理的答案。真正能分辨這兩種可能的證據要等 Act III 的 `cairn_access_log_extract.txt` 跟 root 文件才會出現。
...
- `training_roster_fragment.txt` — **只給訓練代號 + 殖民地 + 年齡，不給姓名**，見 4.5 節。
...
**為什麼「LONGSHORE = Naomi Okafor」是合理的推論，不只是同名的巧合**：這條線完全是 optional evidence chain（不影響主線攻擊鏈，玩家不需要找到它也能過關），但既然遊戲把它放進去當作一條可選的推理節點，這裡的邏輯也要站得住腳：
- **正面證據，三個獨立來源疊在同一個人身上**：(1) relay `system_migration_log` 的「N. Okafor, Colonial Records Clerk」在 2547 年處理的**正好是**包含 Eli 案件在內的那批 Eridanus II/Madrigal 重新索引作業；(2) `dependent_notification_fragment.txt` 顯示 Eli 案件的**登記監護人全名**是「Naomi Okafor」；(3) `foia_review_2553.txt` 顯示 2553 年提出申請的 N. Okafor，申請的對象**正好又是**同一筆 Eli 的案件。三個文件各自獨立存在（不同年份、不同系統、不同目的），但全部指向同一個案件編號，而且姓氏+職務／姓名的組合完全對得上——如果是三個互不相關的人，代表巧合疊了三層：一個無關的記錄員恰好處理到 Eli 的案子、一個無關的申請人恰好用同一個案件編號申請、監護人姓名又恰好跟前面兩者同名。單一解釋（三份文件講的是同一個人）需要的巧合遠比三個無關的人疊在一起少，這是最基本的證據權重判斷，不是「感覺對得上」。
- **為什麼「官方自己都說無關了」不能當作反證**：`foia_review_2553.txt` 裡 Compliance 官員 S. Andrade 的判定依據寫的是「per personnel records on file」——這是對照**人事系統**查有沒有正式登記的親屬關係，而監護人資格登記在**被扶養人案件系統**裡，兩套系統本來就不會自動互相參照（除非有人手動去查）。官員的「無關」判定是一次淺層的資料庫比對沒找到自動關聯，不是真的去比對過這兩個系統各自記載的實際內容——玩家手上同時握有這兩套系統各自的內容（案件系統的監護人姓名 + 人事/流程紀錄的記錄員姓名），官員沒有這個條件同時看到兩邊。
- **這條線本身也誠實承認自己的極限**：`truth-map.md` 明講這是「不強制玩家發現」的身分揭露，遊戲從頭到尾不會用文字直接說「LONGSHORE 就是 Naomi Okafor」——因為就算證據權重上合理，這終究是循環佐證出來的推論而非鐵證（例如：真的存在極端巧合的可能性沒有被邏輯排除，只是機率極低）。這正是本節點該有的分寸：證據夠強，足以讓玩家自己下判斷，但遊戲不會替玩家把這個推論包裝成不容置疑的事實。
...
**這個節點跟 4.5/4.5b 不一樣，它不是一個需要排除替代解釋的推論**：105 的表格是直接證據，不是要玩家去推測——第四筆（`OCPA-R4-10733`）的 ONI 端審核人欄位白紙黑字寫著「R. Achebe」，不是「大概是別人」這種要靠間接證據拼湊的結論。玩家要做的推理工作只有一步：注意到 102 只講「三份」卻不講是哪三份，然後自己去 105 對出這三份是哪幾筆、第四筆又是誰——推理的困難點在「有沒有主動去比對」，不在「證據夠不夠」，證據本身是完整且無爭議的。
...
**為什麼「07-B = Farrow」是合理的推論，不是感覺上湊得起來就算數**：
- **正面證據**：殖民地（Skopje）跟徵召年齡（6 歲）兩個獨立資料點完全吻合，而且這是玩家手上僅有的兩個可比對欄位——不是只挑一項符合。Kade 對 07-B 的描述帶著具體、私人的情感記憶（「我帶過最凶的六歲小孩」），跟他自述「負責的只是 SPINDLE 名單裡一小群，不是整個梯隊」吻合：07-B 是他真的「親自從頭訓練到底」的少數幾人之一，不是隨口提一個他其實不熟的名字。
- **為什麼「可能是另一個沒被記錄到的 Skopje 候選人」站不住腳**：這批徵召一共 75 人，理論上不能百分之百排除還有別的 Skopje 孩子存在——但玩家能接觸到的每一份文件（案件卡、六份 CAIRN 紀錄、三個 SMB 分享）裡，**沒有任何一處出現第二個 Skopje 案例，也沒有任何一份文件提示過這種可能性**。這款遊戲對「真正無法確定」的節點（3.4 節官方稽核矛盾、4.5b 節 Farrow 的結局）一律會**至少留兩份文件互相打架**，讓矛盾是玩家自己讀出來的，不是憑空想像出來的；07-B 這裡完全沒有這種互相矛盾的線索並存——這種「沒有矛盾線索」的狀態本身，在這個遊戲一貫的設計邏輯下，就是「這裡不是一個刻意留白的節點」的訊號。換句話說：遊戲不會無中生有引入一個玩家永遠查不到、也不會有任何文件暗示存在的競爭對象，來讓一個原本清楚的推論落空。
- **這個節點解決的是「07-B 是誰」，不是「07-B 後來怎麼了」**：身分還原到此為止就結束，「後來怎麼了」是下一個獨立的節點（4.5b），三份來源互相矛盾、遊戲刻意不解答——身分確定，結局不確定，這是兩件事，不要混在一起判斷。
...
**為什麼這三份來源沒有辦法互相裁決**：三份都是各自領域裡合理、可信的一手來源——官方 casualty log 是制度性紀錄（沒有明顯動機造假，但制度性紀錄本來就可能因為行政便宜行事而失真）；Kade 是在場目擊者、事後多年才寫、明確標註「反正沒人會看」，沒有明顯動機說謊，但人的記憶跟情感投射本身就不是絕對可靠的證據；低溫轉移授權是同批次的醫療行政文件，時間點吻合，但它自己都承認「recovery team 選擇直接進入懸置，未記錄正式的現場判定結果」——連它自己都不是一份完整、經過正式判定的紀錄。三者互相牴觸，但沒有任何一份能被另外兩份**證偽**（不是「兩份說一樣、一份不同就採多數」——那不是證據權重，是計票，而這裡的情況剛好是三份各說各話，連多數決都套不上）。root 文件裡 Petrov 自己也講了同一件事：他讀過三份、順序都讀過不只一次，也沒有打算「在這裡再寫第四個版本」——這不是遊戲偷懶，是誠實反映「有些事在現有證據下就是查不出來」，這正是這個節點存在的意義。
