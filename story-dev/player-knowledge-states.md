...
仍不知道：官方最終處置決定「背後的理由」（Disposition Order 已知結論，但還沒看到 Petrov 自己為什麼這樣決定）。

## ARCHIVE root 之後（讀到 cairn_disposition_review.txt）
新增知道：**只有一件新事情**——Petrov 為什麼決定保留這批資料而不銷毀，以及他自己都承認「沒有機制可以決定要不要公開」。
文件**不會**重講一次起源、掩蓋手法、訓練與 augmentation 結果——那些玩家應該已經從 Act II/III 的其他文件裡自己拼出來了。Farrow 的結局也**不會**在這裡被解答。
